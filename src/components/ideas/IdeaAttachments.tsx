...
export function IdeaAttachments({ ideaId }: Props) {
  const { user } = useAuth();
  ...
  const handleDelete = async (att: Attachment) => {
    await supabase.storage.from('idea-attachments').remove([att.file_path]);
    await supabase.from('idea_attachments').delete().eq('id', att.id);
    fetchAttachments();
  };
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {attachments.map((att) => (
          <div key={att.id} className="flex items-center gap-2 rounded-lg bg-muted/50 p-2 text-sm">
