    ...
    {[
      ...
      <div key={col.title} className="rounded-lg border border-border bg-muted/30 p-3">
        ...
        <div className="space-y-2">
          {col.cards.map((card) => (
            <div key={card} className="rounded-md border border-border bg-card p-2.5 text-xs shadow-sm hover:shadow-md transition-shadow cursor-grab">
              {card}
            </div>
          ))}
        </div>
    ...
    <div className="flex gap-3">
      {[
        ...
        <div key={b.label} className="flex flex-col items-center gap-1">
          <div className="h-10 w-10 rounded-full border border-border bg-card flex items-center justify-center hover:bg-muted transition-colors cursor-pointer">
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        ...
        {Array.from({ length: 30 }, (_, i) => {
          ...
          const ev = events[day];
          return (
            <div key={day} className={`relative rounded-md p-1 text-xs ${day === 15 ? 'bg-primary/10 font-bold' : 'hover:bg-muted'} cursor-pointer`}>
              {day}
